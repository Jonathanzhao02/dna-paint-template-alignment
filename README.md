...
More details about each of the above steps are found below.
...
To use this script, run: `python3 process_clusters_kmeans.py <path to dataset folder> [-o <output file name>] [-t <template name>] [-c <clustering method>] [-a <alignment method>] [-j <path to config file>] [-i <path to clusters file>] [-o <path to output folder>]`. Arguments in square brackets are optional, arguments in angled brackets are required.
